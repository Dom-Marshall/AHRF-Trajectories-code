...
# ---------------- Output structure -----------------------------------------
root_dir <- "Class prediction validation v5"
...
static_mim <- long_mim %>% select(stay_id, age) %>% distinct()
...
static_icht <- long_icht %>% select(stay_id, age) %>% distinct()
...
long_aumc <- long_aumc %>%
  ...
  mutate(
    # Use imputed age for static features
...
# Binary – recal (site-trained) with fixed 0.5
...
# Recal (site trained) binary at 0.5
