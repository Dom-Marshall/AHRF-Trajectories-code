...
#   MIMIC-IV  - referred to as CRLCMM in the paper
# ===============================================================
...
cfg <- list(
  ...
  admin_day        = 14,
  k_range          = 2:2,
  rep_starts       = 20, # recommend doing parameter search and stopping when models stabilise (original search up to 200)
  max_iter         = 200,
  parallel_type    = "PSOCK", # need to change if computer cluster vs windows pc
  seed_global      = 123
)
# ---------------------------------------------------------------------------
...
# -------- 1. LOAD & HARMONISE ----------------------------------------------
pf_daily <- read.csv(cfg$csv_pf)        # ← has days_from_start already (of AHRF onset)
stays    <- read.csv(cfg$csv_stay)      # ← stay_id, timezero, outtime, Mortaliy flag
...
print(check_tab)
write.csv(check_tab, file.path(cfg$out_dir, "SanityCheck_D14_counts.csv"), #Important!
          row.names = FALSE)
